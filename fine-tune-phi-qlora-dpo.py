...
parser.add_argument('--save_root',
                    type=str,
                    help='Root directory to store saved results.')
...
parser.add_argument('--curr_epoch',
                    type=int,
                    default=0,
                    help='Current epoch for online distillation.')
...
#online_status = "online" if (is_online or curr_epoch !=0) else "offline"
online_status = "online" if num_epochs == 1 else "offline"

root_dir = args.save_root
input_dir = f'{root_dir}/code_{dataset}/{online_status}_lr={lr}_e={curr_epoch-1}'
output_dir = f'{root_dir}/code_{dataset}/{online_status}_lr={lr}_e={curr_epoch}'
os.makedirs(output_dir, exist_ok = True)
if(not os.path.exists(input_dir)): input_dir = PHI_2_MODEL_ID
...
#                                           add_eos_token=True,
#                                           use_fast=True)
tokenizer = AutoTokenizer.from_pretrained(input_dir, add_eos_token=True,  use_fast=True)
tokenizer.padding_side = 'right'
tokenizer.pad_token = tokenizer.eos_token

# Json data is in an incorrect form -- need to switch "rows" and "columns"
if(curr_epoch!=0):
    json_file = open(f'{input_dir}/result.json')
...
data_list = []
rejected_key = 'rejected' if curr_epoch ==0 else 'trained_output'
for i in range(len(json_data['prompt'])):
    sample = {}
    sample['prompt'] = json_data['prompt'][i]
    sample['chosen'] = json_data['chosen'][i]
    sample['rejected'] = json_data[rejected_key][i]
    if(sample['rejected']!=None): # 3 examples encountered error during data generation
...
# )
model = AutoModelForCausalLM.from_pretrained(input_dir, quantization_config=bnb_config, trust_remote_code=True)
...
if not is_online:
    ...
    raise Exception('online training can only be done by calling offline mode multiple times')